...
const calculateTotalCasesBySex = (info_ICS: CaseEntry[]) => {
  const totalCasesBySex = {
    male: 0,
    female: 0
  };

  info_ICS.forEach((entry: CaseEntry) => {
    if (entry.Sexe === 'H') {
...
const HomePage = async() => {
  ...
  const info = data && data.info_ICS ? data.info_ICS : []
  const info_ICS = calculateTotalCasesBySex(info);
